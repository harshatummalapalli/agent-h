...
function ConditionBadge({ condition }: { condition: SearchIntentCondition }) {
  const cls =
    DISPOSITION_STYLE[condition.disposition] ?? "bg-gray-100 text-gray-800";
  return (
    <span
...
// ─── Approx badge (unenforceable constraints folded into Prefer) ───────────────

function ApproxBadge({ constraint }: { constraint: UnenforcedConstraint }) {
  const cls = DISPOSITION_STYLE["prefer"] ?? "bg-blue-100 text-blue-800";
  return (
    <span
      className={`inline-flex items-center gap-1 rounded px-2 py-0.5 text-xs font-medium ${cls}`}
      title={constraint.reason ?? constraint.description}
    >
      <span className="font-semibold">{constraint.description}</span>
      <span className="text-[10px] opacity-60 font-normal">approx.</span>
    </span>
  );
}
...
function DispositionGroup({
  ...
  unenforceables?: UnenforcedConstraint[];
}) {
  if (conditions.length === 0 && unenforceables.length === 0) return null;
  return (
    <div className="flex flex-wrap gap-1.5">
      ...
      {unenforceables.map((u, i) => (
        <ApproxBadge key={`approx-${i}`} constraint={u} />
      ))}
    </div>
  );
}
...
export function SearchIntentDisplay({
  ...
  const required = current.conditions.filter(
    (c) => c.disposition === "require",
  );
  const excluded = current.conditions.filter(
    (c) => c.disposition === "exclude",
  );
  const preferred = current.conditions.filter(
    (c) => c.disposition === "prefer",
  );
  const unenforceable: UnenforcedConstraint[] =
    current.unenforceable_constraints ?? [];
  ...
  return (
    <div className="space-y-3">
      {/* Conditions by disposition — unenforceables folded into Prefer */}
      <div className="space-y-2">
        <DispositionGroup disposition="require" conditions={required} />
        <DispositionGroup disposition="exclude" conditions={excluded} />
        {/* Prefer always renders when unenforceables exist, even if no hard prefer conditions */}
        <DispositionGroup
          disposition="prefer"
          conditions={preferred}
          unenforceables={unenforceable}
        />
      </div>
