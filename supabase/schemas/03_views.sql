--
-- Views
-- This file declares all views in the public schema. Unmodified from
-- Atomic CRM -- these still work as-is on top of the tenant-scoped tables,
-- since they read through whatever RLS already allows the querying user to
-- see (security_invoker = on means the view runs as the caller, so tenant
-- isolation is inherited automatically, not bypassed).
--
